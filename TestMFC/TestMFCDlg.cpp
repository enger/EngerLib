...
{
	typedef void (_stdcall * StartFunc)(string , unsigned short);//定义函数指针类型
	typedef void (_stdcall * StopFunc)();//定义函数指针类型

	HINSTANCE hInst;

	hInst=LoadLibraryA(szFunc);//动态加载Dll

	StartFunc Start=(StartFunc)::GetProcAddress(hInst,"Start");//获取Dll的导出函数
